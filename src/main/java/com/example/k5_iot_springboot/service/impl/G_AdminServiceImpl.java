...
public class G_AdminServiceImpl implements G_AdminService {
    ...
    public ResponseDto<RoleManageResponse.UpdateRolesResponse> replaceRoles(
            UserPrincipal principal, RoleManageRequest.@Valid UpdateRolesRequest req) { // 1) 갱신될 사용자 정보 조회
        G_User user = userRepository.findWithRolesById(req.userId())
                .orElseThrow(() -> new EntityNotFoundException("해당 id의 user가 없습니다."));

        // 2) 전체 교체 - 요청에 @NotEmpty이므로 최소 1개 이상의 권한을 보장!
...
//        req.roles().forEach(user::addRole);
    ...
    @Override
    @Transactional
    public ResponseDto<RoleManageResponse.AddRolesResponse> addRoles(
        ...
        RoleManageResponse.AddRolesResponse data = new RoleManageResponse.AddRolesResponse(
                user.getId(),
                user.getLoginId(),
                req.role(),
    ...
    // 권한 제거 (권한 회수 - 최소 1개 권장 보장: 모두 제거되면 USER 유지)
    @Override
    @Transactional
    public ResponseDto<RoleManageResponse.RemoveRolesResponse> removeRoles(
...
//        }
        ...
        RoleManageResponse.RemoveRolesResponse data = new RoleManageResponse.RemoveRolesResponse(
                user.getId(),
                user.getLoginId(),
                req.role(),
                Set.copyOf(user.getRoleTypes()),
