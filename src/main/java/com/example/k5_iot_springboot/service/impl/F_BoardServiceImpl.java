...
public class F_BoardServiceImpl implements F_BoardService {
    ...
    private Pageable buildPageable(int page, int size, String[] sortParams) {
        // 정렬 파라미터 파싱(스플릿): ["createdAt, desc", "title,asc"] 형태
        ...
        if (sortParams != null && sortParams.length > 0) { // 빈 배열이 아닌 경우 (요소 1개 이상)
            ...
            for (String p: sortParams) {
                ...
                // 기본 정렬 방향은 DESC - 피드/게시물은 최신순 정렬이 일반 (권장)
                if (t.length > 1) { // 정렬기준이 존재함
    ...
    @Transactional
    @Override
    public ResponseDto<BoardResponseDto.DetailResponse> createBoard(BoardRequestDto.@Valid CreatedRequest request) {
    ...
    @Override
    public ResponseDto<List<BoardResponseDto.SummeryResponse>> getAllBoards() {
        List<F_Board> boards = boardRepository.findAll();

        List<BoardResponseDto.SummeryResponse> result = boards.stream()
                .map(BoardResponseDto.SummeryResponse::from)
                .toList();
    ...
    public ResponseDto<BoardResponseDto.DetailResponse> updateBoard(Long boardId, BoardRequestDto.@Valid UpdateRequest request) {
        ...
        //      - 영속성 컨텍스트가 DB에 반영될 때
        //      >> 서비스 안에서 DTO 변환이 곧바로 일어날 때 updateAt이 갱싱 전 값으로 보여짐
        //      +) 다시 실행 시 커밋된 변경사항 확인가능
        ...
        boardRepository.flush(); // 변경 내용을 DB에 반영 (@PreUpdate 트리거 >> updateAt 채워짐)
...
//        Pageable pageable = buildPageable(page, size, sort);
        ...
        // 2) Slice<T>
        //      : count 쿼리 실행 X, 데이터 개수를 size + 1로 오쳥해서 다음 페이지 존재 여부만 판단
        ...
        //      >> 특징
        //          : 실제 구현체는  PageRequest 사용 (PageRequest.of())
        //          : JpaRepository의 findAll(Pageable pageable) 메서드 전달


        Page<F_Board> pageResult = boardRepository.findAll(pageable);

        List<BoardResponseDto.SummeryResponse> content = pageResult.getContent().stream()
                .map(BoardResponseDto.SummeryResponse::from)
                .toList();
    ...
    public ResponseDto<BoardResponseDto.SliceResponse> getBoardsByCursor(Long cursorId, int size) {
        ...
        List<BoardResponseDto.SummeryResponse> content = slice.getContent().stream()
                .map(BoardResponseDto.SummeryResponse::from)
                .toList();
