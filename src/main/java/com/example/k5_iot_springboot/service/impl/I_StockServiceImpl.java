...
public class I_StockServiceImpl implements I_StockService {
    ...
    public ResponseDto<StockResponse.Response> adjust(
            ...
            StockRequest.@Valid StockAdjust req) {
        // 재고 증감 (delta)
        // : delta 값이 양수면 - 입고/반품
        // :       값이 음수면 - 출고/차감

        StockResponse.Response data = null; // 실제 ResponseDto 내부에서 전달될 data 타빙을 초기화
    ...
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    @Transactional
    public ResponseDto<StockResponse.Response> set(
            UserPrincipal userPrincipal,
            StockRequest.@Valid StockSet req) {
