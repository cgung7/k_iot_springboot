...
/*
    ...
    필요성
    1) Security의 표준 진입점: AuthenticationProvider(DAO)는 UserDetails 타입을 통해 사용자 정보와 권한을 검사
            >> CustomUserDetailsService#loadUserByUsername()에서 DB 사용자 정보를 읽고
                    해당 클래스로 감싼 뒤 반환하면, 이후 인증 과정이 표준화되어 동작

    2) 경량/안정성 향샹: 영속성 엔티티(G_User)를 SecurityContext에 보환하면
                        , 직렬화 문제, 지연로딩, 순환 참조 등의 문제 발생 가능성 증가
                        >> VO 형태의UserPrincipal은 인증에 필요한 최소 데이터만 포함하고 있어 안전
    ...
    >> 권한 모델 (authorities): GrantedAuthority 집합
        EX) new SimpleSimpleGrantedAuthority("ROLE_USER")
        - 스르핑 시큐리티의 hasRole("USER") / hasAuthority("ROLE_USER") 검사와 호환되도록 "ROLE_" 접두어를 붙이는 것을 권장


    >> 설계 포인트
    1) 불변성: 모든 필터는 final (생성 이후 변경 불가)
    2)@JsonIgnore, @ToString(exclude="password")를 통해
...
public class UserPrincipal implements UserDetails {
    ...
    //              >> Spring Security가 사용자의 정보를 불러오기 위해서는 UserDetails를 구현해야 한다.
    ...
    private final boolean enabled;                                          // 활정화 여부
    ...
    // 계정의 비밀번호 리턴
    // >> 인증 단계에서 DaoAuthenticationProvider가 비밀번호 매칭에 사용 (반드시 해시값으로)
