...
public interface G_UserRepository extends JpaRepository<G_User, Long> {
    ...
    // : u.roles 컬렉션을 한 번에 가져오기 때문에 N+1 문지방지
    @Query("""
        SELECT u
        FROM G_User u
    ...
    """)
    Optional<G_User> findWithRolesByLogIn(@Param("loginId") String loginId);

    // 해결 방법 2) JPA의 @EntityGraph을 사용하여 fetch join을 자동적으로 적용 방식
