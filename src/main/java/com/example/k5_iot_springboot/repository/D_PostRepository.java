...
/*
    ...
    -- 2번째 쿼리: 이후 post.getComments() 차음 호출 시
...
public interface D_PostRepository extends JpaRepository<D_Post, Long> {
    ...
    // +) 8번(댓글이 가장 많은 상위 N개)
    //      : 쿼리 메서드만드로는 집계/정렬 불가 >> JPQL 또는 Native Query 사용
    ...
    // [2] 인터페이스 프로젝션 (권장)
    //      >> 결과 컬럼 별칭 <-> 인터페이스 Getter 이름 매칭으로 타임 세이프 (캐스팅 불필요)
