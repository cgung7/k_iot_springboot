...
//        return ResponseEntity.ok(result);
    ...
    // 공통점: 클라이언트 부터 값을 받는 방법
    // != 값을 받는 위치와 의미가 다름

    // @RequestParam
    // : URI의 ? 이후에 있는 key=value 형태의 값을 가져옴
    // - 검색, 필터링, 정렬, 페이지네이션 등 조건을 전탈할 때 사용
    //      >> 값이 선택적일 수 있음 (request=false 설정가능 - 필수 X)
    //      >> 여러 개 받을 수 있음 (&로 나열)
    ...
    // 경로값에 ? 이후의 데이터를 키-값쌍으로 추출되는 값 (?키 =값)
    // > 항상 문자열로 반환(숫자형은 int, long으로 자동변환)
