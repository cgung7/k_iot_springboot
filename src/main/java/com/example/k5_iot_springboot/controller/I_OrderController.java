...
public class I_OrderController {
    ...
    public ResponseEntity<ResponseDto<OrderResponse.Detail>> create(
            // 매개 변수 - Controller (@PathVariable, @RequestBody, @RequestParam)
            @AuthenticationPrincipal UserPrincipal principal,
            @RequestBody OrderRequest.OrderCreateRequest req
    ) {
        ResponseDto<OrderResponse.Detail> response = orderService.create(principal, req);
//        return ResponseEntity.ok(response);   <- 간결하게 사용 가능(기타 클라이언트 정보 제공이 필요할 경우 X)
    ...
    /* 주문 취소: USER(본인 + PENDING 한정), MANAGER(매장 사장님), ADMIN(본사 관리자)  */
    ...
    /* 주문 검색: USER(본인), MANAGER(매장 사장님), ADMIN(본사 관리자)  */
    @GetMapping
