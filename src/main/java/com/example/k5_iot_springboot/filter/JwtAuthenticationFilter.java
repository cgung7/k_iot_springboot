...
/*
    ...
    : JWT 인증 필터
    - 요청에서 JWT 토큰을 추출
    - request header에서 토큰을 추출하여 검증 (유효한 경우 Security의 context에 인증 정보 설정)
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            // 이미 인증된 컨텍스트가 있으면 스킵 (다른 필터가 인전처리를 한 경우)
            ...
            if (!jwtProvider.isValidToken(token)) {
                filterChain.doFilter(request, response); // 토큰이 유효하지 않는 경우 - 시큐리티 설정 없이 로질 실행
                return;
            }
