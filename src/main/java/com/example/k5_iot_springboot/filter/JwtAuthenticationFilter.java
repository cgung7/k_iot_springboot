...
/*
    ...
    : JWT 인증 필터
    - 요청에서 JWT 토큰을 추출
        >>  request header에서 토큰을 추출하여 검증 (유효한 경우 SecurityContext에 인증 정보 저장)
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            // 0) 사전 스킵 조건: 이미 인증된 컨텍스트가 있으면 그대로 진행(스킵) (다른 필터가 인전처리를 한 경우, 중복 인증 방지)
            if (SecurityContextHolder.getContext().getAuthentication() != null) {
                // 현재 스레드(요청) 컨텍스트에 이미 인증 정보가 들어있는지 확인
                // - 자른 필터가 먼저 인증을 끝낸 경우 굳이 중복 인증 X >> 다음으로 진행
            ...
            // 토큰이 유효하지 않는 경우 - 시큐리티 설정 없이 로직 실행
            if (!jwtProvider.isValidToken(token)) {
                unauthorized(response, "토큰이 유효하지않거나 만료되었습니다.");
                return;
            }

            // 7) 사용자 식별자 & 권한 추출
            String username = jwtProvider.getUsernameFromJwt(token);
            Set<String> roles = jwtProvider.getRolesFronJwt(token);

            // 8) 권한 문자열 - GrantedAuthority로 매핑 ("ROLE_" 접두어 보장)
            // : 스프링 시큐리티가 이해하는 권한 타입으로 변환  Collection<? extends GrantedAuthority> 'authorities'
    ...
    private void setAuthenticationContext(
        ...
        // - 두 번째 인자 Credentials (이미 토큰 검증을 마쳤으므로 null로 전달, 중복 검증 필요X)
        // - 세 번째 인자 권한 목표
        // >>"username이라는 사용자가 authorities 권한으로 인증됨" 상태가 됨

        // cf) 권한이 있는 경우(비워지지 않는 겅우) - isAuthenticated = true
        AbstractAuthenticationToken authenticationToken =
                new UsernamePasswordAuthenticationToken(username, null, authorities);

        // 요청에 대한 세부 정보 설정
        // : 생성된 인증 토근에 요청의 세부사항 성절 (예: 원격 IP, 세션 ID 등)
        authenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));

        // 빈 SecurityContext 객체 생성 - 인증 토큰 주입
        SecurityContext context = SecurityContextHolder.createEmptyContext();
        context.setAuthentication(authenticationToken); // 방금 만든 인즌 토큰을 달아둠
    ...
    private List<GrantedAuthority> toAuthorities(Set<String> roles) {
        ...
        // cf) "ROLE_" 첨부 이유
        // 스프링 시큐리티의 기본 hasRole("권한")은 내부적으로 ROLE_가 첨부돤 권한 문자열을 찾음
