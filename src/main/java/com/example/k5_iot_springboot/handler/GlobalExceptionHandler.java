...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<ResponseDto<Object>> fail(
        ...
        // 1) reason값 설정: 비워질 경우 ErrorCode의 기본 메세지 값 사용
    ...
    public ResponseEntity<ResponseDto<Object>> handleAccessDenied(EntityNotFoundException e) {
        log.warn("Not Found: {}", e.getMessage());
        return fail(ErrorCode.NOT_FOUND, null, null);
    }

    // === 409 Conflict: 무결성 위반(중복/제약조건) === //
    @ExceptionHandler(DataIntegrityViolationException.class) // Unique 키 출돌, FK 위반 등
