...
/*
    ...
    >> 서버 간의 통신을 원활하게 처리
    - URL 별 접근 권한, 필터 순서(JwtAuthentication를 UsernamePasswordAuthenticationFilter 앞에 배치) 등
...
public class WebSecurityConfig {
    ...
    @Value("${cors.allowed-origins:*}") // http://app.example.com, http://admin.example.com
...
/*
    ...
    : 특정 출처에서 온 HTTP 요철을 허용하거나 거부할 수 있는 필터
    ======================
 */
@Bean
...
//    config.setAllowedOriginPatterns(origins);                   // 2) Origin 설정 - 도메인 매칭
    ...
    config.setExposedHeaders(splitToList(exposedHeaders));      // 5)응답에서 클라이언트가 읽을 수 있는 헤더(Authorization, Set)

    UrlBasedCorsConfigurationSource source = new UrlBasedCorsConfigurationSource();
    source.registerCorsConfiguration("/**", config); // 모든 경로에 동링 CORS 정책 적용
    return source;
}
    /*
        ======================
...
//                -> headers.frameOptions(HeadersConfigurer.FrameOptionsConfig::sameOrigin));
        ...
        http
                ...
                // H2 콘솔 접근 권한 열기 (개발 환경에서 DB를 직접 확인 - 인증 정차 없이 접속할 수 있도록 예외)
