...
// 아무런 매개변수가 없는 기본 생성자를 Protected 접근제어자로 생성
// : 외부 클래스에서(엔티티 정의 부분 외) 객체를 직접 생성하는 것을 방지
// - 무분별한 객체 생성을 억제
// cf) JPA에서 엔티티 생성 시 기본 생성자가 반드시 필요
//      >> JPA를 위한 생성자 프로젝트 전역에서 접근 하는것을 방지
public class H_Article extends BaseTimeEntity {
    ...
    @ManyToOne(fetch = FetchType.LAZY, optional = false) // Article은 Many , User 는 One으로
    // fetch: 가저오다 , LAZY: 게으른
    // -> 연관된 엔티티를 필요할 때만 DB에 로딩(전달)
