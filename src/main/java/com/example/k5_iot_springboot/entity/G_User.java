...
public class G_User extends BaseTimeEntity {
    ...
    @Column(name = "password", nullable = false, length = 255) // 암호화로 255설정
    ...
    @CollectionTable(
            name = "user_roles",
            joinColumns = @JoinColumn(name = "user_id", foreignKey = @ForeignKey(name = "fk_user_roles-user"))
            ,
...
/*
    User 엔티티
    - 테이블(users)와 1:1 매핑
    - 생성/수정시간은 BaseTimeEntity에서 자동 세팅
