...
/*
    ...
    2. 서블릿 동작 구조
        ...
        [ 웹 서버 / 서블릿 컨테이너 ] 톰캣, 제티 등
            |
            | : URL 매칭 ( "/hello" 경로 >> HelloServlet 클래스)
            |
        ...
        === 서블릿 특징 ===
        1) 서블릿 컨테이너가 서블릿의 생명주기를 관례
            into(): 최초 1회 초기화
            service(): 매 요청마다 호출 ?? 내부에서 doGet/doPost() 등으로 분기
            destroy(): 컨테이너 종료 시 정리

        2) 매핑(web.xml 또는 어노테이션 @WebServlet)으로 어떤 URL이 어떤 서블릿으로 연겯되는지 개발자가 명시
        3) 순수 서블릿은 요청 파싱, 모델 구성, 뷰 렌더링을 개발자가 직접 처리 - 중복 코드 증가

        >> 순수 서블릿의 불편함을 표준화/자동화 해주는 프론트 컨트롤러 패턴의 구현이
