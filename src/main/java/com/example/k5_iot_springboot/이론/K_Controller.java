...
/*
    ...
    >> GET, PUT[UPDATE], DELETE 사용 (POST[CREATE] 사용하지 않는다)

    1) 리소스를 특정할 수 있는 PK 값을 주로 사용
    2) 경로 내에 {}로 값을 감싸서 표현
    3) {} 내의 데이터가 @PathVariable 뒤에 매개변수와 매핑 - Long isbn
    ...
    2) 클라이언트(프론트 엔드)는 "Content-Type: application/json" 헤더 설정 필요
    ...
    - URL에 노출되기 때문에 민감하지 않는 데이터에 적합
    - 주로 GET 요청에 주로 사용

    - 간단한 검색 조건, 필터링 & 페이징 가능, 보안이 크게 중요하지 않는 데이터
