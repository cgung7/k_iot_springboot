...
/*
    ...
    2. 인가(Authorization)
        ...
        - 관리자만 특정 페이지에 접근 가능하거나, 특정 데이터를 호출할 수 있도록 설정

    cf) CSRF(Cross-Site Request Forgery)
        : 자신(인증돤 사용자)의 의지와 상관없이  특정 웹 애플리케이션에
            , 공격자가 원하는 요청을 보내도록 유도하는 웹 보안 취약점

    === Security 사용 목적 ===
    1. 보안 기본기 자동 제공: 비밀번호 암호화, 세션/토큰 인증, 권하 체크(+인가), CSRF 보호 등
    2. 표준화된 구조: 보안 코드가 일관되고 유지보수 쉬움
    3. 유연한 확장성: 소셜 로그인, OAuth2, JWT 토큰, 2FA 등으로 확장 가능.
    ...
    2. 세션(Session) VS 토큰(JWT)
        1) 세션: 서버에 "누가 로그인 했는지" 기록. 클라이언트는 세션 ID 쿠키만 전달 // 키워드로 사용자 명부 저장 - 기록 //
            - 서버가 로그인 상태 기억
        2) JWT: 서버는 토큰만 검증하고 별도의 저장소가 없어도 됨 (완전한 Stateless) // 토큰으로 DB에 확인 후 진행 요청 - 저장 X //
            - 서명된 토큰으로 인증
   ...
   >> 모든 요청은 필터 체인을 통과
   >> 인증이 필료한 URL이면 로그인 정보 화긴 + 성공 시 SecurityContext에 사용자/권한을 저장
