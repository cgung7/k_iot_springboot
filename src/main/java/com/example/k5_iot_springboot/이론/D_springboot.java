...
/*
    ...
    - 내장 WAS(Tomcat cat)를 포함: 별도 설치 없이 실행 가능
    ...
    3) DX >> 핸들러 어댑터
        ...
        >> 어댑터
            : 스프링은 컨트롤러 파일이 달라고 공통 방식으로 호출 할 수 있도록 어댑터가 중간에서 표준화
            (@Controller, @RstController 등)
...
/*
    === 서블릿(Servlet) VS 스프링 부트(Spring Boot) ===
    1. 서블릿
       1) 각 서블릿이 컨트롤러 역할까지 직접 함 - 구조를 스스로 만들고, 규칙도 스스로 정해야 함
       2) 기능/URI 마다 여러 서블릿을 만들거나, 직접 프론트 컨트롤러 패턴(서블릿 1개 + 내부 분기)를 구현
    ...
    2. 스프링 부트(MVC)
        ...
        2) 서블릿 레벨이 DispatcherServlet 1개면 충분 (스프링이 자동 등록)
            - 컨트롤러 클래스/매서드는 여러 개
                ...
                cf) POJO 메서드
                    : 클래스 내부에 정의된 메서드를 의미
                    - 특정 프레임워크나 기술에 종속되지 않고 순수하게 데이터와 로직을 갭슐화하는 객체를 의미
