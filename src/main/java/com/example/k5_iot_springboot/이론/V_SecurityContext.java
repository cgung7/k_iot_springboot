...
/*
    ...
    4. 사용자 로딩 (UserDetailsService)
        ...
        - 조회가 실패) UsernameNotFoundException 발생 ("아이디/비밀번호가 올바르지 않음"으로 외부 전달)
    ...
    7. 계정 상태 점검
        - isAccountNonLocked, isEnabled, isAccountNnExpired, isCredentialsNinExpired
        - 하나라도 false면 각각의 예외 발생 (잠근, 비활성, 만료 등)
    ...
    10. JWT 발급 (애플리케이션 레벨)
        ...
        - 클래임 sub=username, roles, iat, exp
    ...
    11. 응답 작성
        ...
        ---------------
        +) 이후 요청 처리 (인가 단계)
            - JWT 필터라 헤더의 토큰 검증 >>  유효하면 UserPrincipal 재구성 >> SecurityContext에 주입
 */
