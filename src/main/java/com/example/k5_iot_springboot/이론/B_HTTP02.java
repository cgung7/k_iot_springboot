...
/*
    cf) 인터넷의 기본 개념
        ...
        2) DNS: 도메인 이름, IP 주소로 변환 (google.com >>> 172.217.@@@.xxx)
    ...
    1. HTTP 메서드 (Method)
    : 요청에 어떤 목적을 가지고 있는지 명시하는 방법

        1) GET(가져오기) - 서버로 부터 데이터를 가져올 것을 요청
        2) POST(생성하기) - 서버에 새로운 데이터를 저장
        3) PUT(수정하기) - 서버의 기존 데이터 '전체'를 수정 (덮어씌우기)
            cf) UPDATE
                >> 서버의 기존 데이터의 일부를 수정
    ...
    : 요청에 대한 처리 결과를 숫자와 간단한 메세지로 알려줌
        ...
        2) 400 - 개발자가 처리해야 하는 문제
            ...
            401 Unauthorized: 인증 실패 (로그인 실패, EX. 사원증 없이 회사 출입 금지, 카페 가입 없이 내용 확인 불가)
            403 Forbidden: 권한 없음 (EX. 회사 내 보안 문서는 권한 없이 열람 불가, 카페 권한 없이는 글쓰기 불가)
            404 NOT FOUND: 요청 자체를 찾을 수 없음 (해당하는 요청이 아에 없음)
