...
/*
    === JpaRepository<E(Entity), ID> ===
    : Spring Data JPA에서 제공하는 기본 인터페이스
    - CRUD와 페이징, 정렬에 포함한 다양한 데이터 엑세스 메서드를 제공
    - 반환타입 메서드명(매개변수);의 추상 메서드 형식

    1. CRUD 메서드
    1) Entity sava(E entity);
    ...
    2) Optional<E> findById(Id id);
        ...
        >> 결과가 존재하지 않는경우 NoSuchElementException 발생 위험 있음
            Optional<> 사용 권장 - orElseThrow() 패턴으로 안전 처리

    3) boolean existsById(Id id);
        : 주어진 Id가 존재하는지 확인(true / false)
