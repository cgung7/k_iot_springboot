...
/*
    ...
    cf) JWT
        ...
        - (클라이언트가) 서버에 '요청'할 때마다 전달 가능 - 토큰 생성(사용자 정보 확인용, Authorization: Bearer <토큰>)
 ...
 */
@Component
// cf) @Component(클래서 레벨 선언) - 스프링 런타임 시 컴포넌트 스캔을 통해 자동으로 빈을 찾고 등록 (의존석 주입)
...
public class JwtProvider {
    ...
    public JwtProvider(
        ...
        this.parser = Jwts.parser()
                .verifyWith(this.key) // 해당 키로 서명 검색을 수행하는 파서 (이후 파싱마다 반복 설정 X)
                .build();
    ...
    /*
        ...
        엑세스 토큰 생성
        @parma username sub(Subject)에 저장할 사용자 식별자
    ...
    public String generateJwtToken(String username, Set<String> roles) {
        long now = System.currentTimeMillis(); // 현재 시간을 밀리초로
        ...
        return Jwts.builder()
                // 표준 클레임(claim) sub(Subject)에 사용자 아이디(또는 고유 식별자) 설정
                .setSubject(username)
                .claim(CLAIM_ROLES, roleList) // 커스텀 클레임 키에 권한 목록 저장 // - payload 저장
    ...
    */
    private Claims parseClaimsInternal(String token, boolean allowClockSkewOnExpiry) {
        ...
        try {
            ...
            // 3) 성공 시 Claims 꺼내기 기능 (.getPayload() 가능)
            // 서명 및 기본 구조 검증 후 페이로드(claims)만 추출해 반환
        }catch (ExpiredJwtException ex) {
            // 토큰이 만료된 경우 발생하는 JJWT 전용 예외 처리
            // : 민료 시간이 지난 토큰에도(예외 안에도) Claims 정보가 들어있음
            if (allowClockSkewOnExpiry && clockSkewSeconds > 0 && ex.getClaims() != null ) {
                ...
                if (exp != null) {
                    ...
                    if (now - exp.getTime() <= skewMs) {
                        // 현재시각 - 만료 시각 <= 허용오차 이내면 "방금 만료" 간주
                        return ex.getClaims(); // 예외에서 Claims를 꺼내 그대로 유효한 것으로 반환
                    }
                }
            }
            throw ex; // 허용 오차 범위를 벗어나면 원랴의 만료 예외를 다시 던짐
    ...
    public boolean isValidToken(String tokenWithoutBearer) {
        ...
        //      서명 불일치, 변조, 포맷 이상, 만료(허용 오차 초과) 등 모든 예외는 catch로 전달 - false 반환
        try {
    ...
    public Claims getClaims (String tokenWithoutBearer) {
        // 유효성 검사 + 파싱을 한 번에 처리하고, payload(Claim) 반환
